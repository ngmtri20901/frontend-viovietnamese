/**
 * Lesson unlock logic for Learn module
 * Determines which lessons are accessible based on user tier and progress
 */

import type { UserLessonProgress } from '../types/exercises'
...
export function checkLessonUnlock(params: {
  ...
  if (userTier === 'FREE') {
    ...
    const allTopicsInPrevZoneCompleted =
      totalTopicsInPreviousZone > 0 && completedTopicsInPreviousZone >= totalTopicsInPreviousZone
...
export function getNextUnlockableZone(params: {
  ...
  const isReadyToUnlock =
    totalTopicsInCurrentZone > 0 && completedTopicsInCurrentZone >= totalTopicsInCurrentZone
